...
namespace Rose.Helper
{
	...
	{
		// Token: 0x0600015F RID: 351 RVA: 0x00019F90 File Offset: 0x00018190
		public static MethodDef copyMethod(this MethodDef originMethod, ModuleDefMD mod)
		{
			short num = 0;
			extension.InjectContext injectContext;
			...
			for (;;)
			{
				if (num == 1)
				{
					injectContext = new extension.InjectContext(mod, mod);
					num = 2;
				}
				if (num != 5)
				{
				...
				IL_131:
				enumerator = originMethod.CustomAttributes.GetEnumerator();
				num = 8;
				goto IL_14C;
			}
			try
